...
>>>>>>> parent of 1b6254f (optimizacion)
============================================ */
...
if (privacyLink && privacyModal && modalClose) {
  ...
  /*
    MODIFICADO: se agrego logica para manejar el display:none correctamente.

    POR QUE: cuando el modal usa display:none, al agregar la clase open
    el navegador aplica display:flex y opacity:1 al mismo tiempo en un
    solo frame, lo que hace que el fade no se vea.

    SOLUCION: al abrir, primero se muestra el elemento (display:flex via clase)
    con opacity todavia en 0, luego en el siguiente frame con requestAnimationFrame
    se activa opacity:1 para que la transicion CSS tenga tiempo de dispararse.

    Al cerrar: se espera a que termine la transicion (transitionend) antes
    de quitar la clase open, asi el fade-out se ve completo antes de ocultarse.
  */
  const openModal = () => {
